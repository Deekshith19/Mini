...
def store_response(url, response, session_number, method="GET", status_code=200, cookies=None, headers=None):
    ...
    with open("main__crawled.json", "w", encoding="utf-8") as f:
...
def main(input_file):
    with open(input_file, 'r') as file:
        ...
        url = lines[1].strip()  # 2nd line is the 1st URL
        username = lines[2].strip()  # 3rd line is the 1st username
        password = lines[3].strip()  # 4th line is the 1st password
    ...
    try:
        ...
        login_url = crawl_and_find_login(base_url, driver, session_number=1)

        if login_url:
            login_to_application(driver, login_url, username, password, session_number=1)
            crawl_with_login(driver, base_url, session_number=1)
        else:
            print(f"Session 1 - No login page found on the website.")
